...
MODULE user_command_0200 INPUT.
  ...
  CASE ok_code.
    WHEN 'SAVE'.
*     ToDo: Save changes
      UPDATE stravelag FROM TABLE gt_travelag_sel. "ct_travelag_sel받은거를 글로벌 변수로 바꾼후 선택 후 저장할 상황이 된 것.
      IF sy-subrc = 0.
        MESSAGE s000(zt03_msg) WITH 'Success'.   "제대로 저장이 됐다. 밑에 메시지가 뜨는 것.
      ELSE.
        MESSAGE s000(zt03_msg) WITH 'not updated records yet'.
      ENDIF.
